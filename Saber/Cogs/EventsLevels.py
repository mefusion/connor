...
class EventsLevels(commands.Cog, name='Уровни'):
    ...
    async def on_message(self, msg):
        ...
        cooldown_stamp = await Exp.find_cooldown(msg.guild.id, member.id)
        ...
        current_xp = await Exp.find_xp(msg.guild.id, member.id)
        ...
        await Exp.update_balance(msg.guild.id, member.id, updated_xp, triggered_at)
        new_xp = await Exp.find_xp(msg.guild.id, member.id)
